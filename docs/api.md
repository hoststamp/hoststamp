...
for the old deterministic profile identity. Token creation accepts optional
`expires_at_ms` as a Unix timestamp in milliseconds. Expired tokens stay
visible in token lists, but they can no longer authorize profile-backed
