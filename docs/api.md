...
admin bearer token, even when general API auth is disabled.
...
basic permissions policy). The CSP permits inline script and style because the
UX is currently bundled as a single static HTML asset.
