...
pub const EFF_WORDLISTS: &[Wordlist] = &[EFF_SHORT_1, EFF_SHORT_2, EFF_LARGE];

pub const EFF_SHORT_1: Wordlist = Wordlist {
    title: "EFF Short Wordlist #1",
    filename: "eff_short_wordlist_1.txt",
    dice: 4,
    source_url: "https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt",
    contents: include_str!("../data/eff/eff_short_wordlist_1.txt"),
};
...
impl Wordlist {
    ...
    pub fn words(self) -> Vec<&'static str> {
        self.entries().map(|entry| entry.word).collect()
    }
...
mod tests {
    ...
    #[test]
    fn eff_short_1_wordlist_has_expected_shape() {
        assert_eq!(EFF_SHORT_1.entry_count(), 1296);
        assert_eq!(
            EFF_SHORT_1.entries().next(),
            Some(WordlistEntry {
                key: "1111",
                word: "acid",
            })
