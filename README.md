...
generation stay public. Profile tokens are generated by Hoststamp, shown once,
and stored only as an HMAC-SHA256 hash keyed by `HOSTSTAMP_TOKEN_HASH_KEY`.
Profile tokens are tied to the immutable profile UUID, so replacing a profile
config invalidates tokens for the old deterministic profile identity.
